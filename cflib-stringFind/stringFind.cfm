<!--- Source: cflib.org | Author: Adam Cameron | https://cflib.org/udf/stringFind --->
<cfscript>
// stringFind(): extends the ability of reFind() & reFindNoPlace that can return ALL the search results (instead of just the first one)
public array function stringFind(required string pattern, required string string, boolean all=false, numeric start=1, boolean caseSensitive=false){
	var result = [];
	var matches = [];
	var i = 0;
	var match = "";
